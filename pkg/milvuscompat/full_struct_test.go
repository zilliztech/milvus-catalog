...
func TestListSegmentsFiltersByPartitionID(t *testing.T) {
	...
	got, err := c.Metadata().Segments().List(
		context.Background(),
		catalog.ListSegmentsRequest{CollectionID: 100, PartitionID: 10},
		catalog.ReadOptions{},
	)
	...
	got, err = c.Metadata().Segments().List(
		context.Background(),
		catalog.ListSegmentsRequest{CollectionID: 100, PartitionID: 10, SegmentIDs: []int64{3, 4}},
		catalog.ReadOptions{},
	)
	require.NoError(t, err)
	require.Len(t, got, 1)
