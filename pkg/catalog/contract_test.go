...
func TestCatalogInterfaceShape(t *testing.T) {
	...
	for _, method := range []string{"Metadata", "AccessControl", "InternalState", "Migration", "Close"} {
...
func TestMetadataInterfaceShape(t *testing.T) {
	...
	for _, method := range []string{"Databases", "Collections", "Partitions", "Aliases", "Segments", "Indexes", "Files", "Snapshots"} {
...
func TestDomainGroupShapes(t *testing.T) {
	...
	stateType := reflect.TypeOf((*MilvusStateCatalog)(nil)).Elem()
	for _, method := range []string{"DataCoord", "QueryCoord", "Streaming"} {
		if _, ok := stateType.MethodByName(method); !ok {
			t.Fatalf("MilvusStateCatalog is missing method %s", method)
		}
	}
}
...
func TestPublicCatalogDoesNotExposeRawKV(t *testing.T) {
	...
	walk(reflect.TypeOf((*MilvusStateCatalog)(nil)).Elem())
